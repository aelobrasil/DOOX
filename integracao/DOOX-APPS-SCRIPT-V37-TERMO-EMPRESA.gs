...
const CONFIG = {
  ...
  // Segurança do Web App: a chave real fica em Script Properties (DOOX_WEB_APP_KEY).
  // Nunca coloque a chave diretamente no código ou no site.
  SECURITY: {
    MAX_BODY_CHARS: 12000,
    MAX_FIELD_CHARS: 500,
    REGISTER_PER_IP_PER_MINUTE: 5,
    REGISTER_PER_IP_PER_10_MINUTES: 20,
    TRACK_PER_IP_PER_MINUTE: 30,
    MAX_INVALID_TOKEN_ATTEMPTS_PER_MINUTE: 10
  },
...
const SHEETS = {
  PEDIDOS: {
    ...
    headers: [
      ...
      'Client Request ID','Token de Acompanhamento','Última Notificação','Benefício HOCCO','Desconto (%)','Cupom HOCCO','Logo da empresa','Termo Empresa','Versão Termo Empresa','Aceite Empresa','Assinatura HOCCO'
    ]
  },
  PAGAMENTOS: {
    name: 'PAGAMENTO',
  ...
  SEGURANCA: {
    name: '_SEGURANÇA',
    headers: ['Data/Hora','Evento','Severidade','Código DOOX','Client Request ID','IP','User-Agent','Ação','Detalhes']
  },
  VEICULACOES: {
    name: '_VEICULAÇÕES',
...
function doGet(e) {
  ...
  try {
    ...
    authorizePublicRequest_(params.key || '');

    const security = {
      ip: String(params.ip || '').trim(),
      userAgent: String(params.ua || '').trim(),
      action: action
    };
    ...
    if (action === 'pedido') {
      ...
      return json_(getPublicOrderStatus_(token, security));
...
function doPost(e) {
  ...
  try {
    ...
    authorizePublicRequest_(body._apiKey || '');

    const security = body._security || {};
    security.ip = String(security.ip || '').trim();
    security.userAgent = String(security.userAgent || '').trim();
    ...
    if (action === 'informarPagamento') {
      return json_(informarPagamento_(body, security));
    }

    // Ações administrativas NÃO ficam expostas ao navegador.
    // O status/pagamento confirmado deve ser manipulado pela operação DOOX
    // na planilha ou por funções internas autenticadas.
    if (['confirmarPagamento','atualizarStatus','publicarObservacao'].indexOf(action) !== -1) {
      throw new Error('Ação administrativa não disponível pela API pública.');
    }
    ...
    return json_(
      registerRequest_(body, security)
    );
...
function informarPagamento_(raw, security) {
...
function registerRequest_(raw, security) {
  ...
  enforceRegisterRateLimit_(security || {});
  if (isHoneypotTriggered_(raw)) {
    logSecurityEvent_(security || {}, 'HONEYPOT', 'ALTO', '', String(raw.clientRequestId || ''), 'Campo oculto preenchido por possível automação.');
    throw new Error('Não foi possível processar a solicitação.');
  }

  const r = normalizeRequest_(raw);
  validateSecurityInput_(r);

  validateRequest_(r);

  logSecurityEvent_(security || {}, 'SOLICITAÇÃO RECEBIDA', 'INFO', '', r.clientRequestId, 'Dados básicos validados pelo backend.');
  ...
  put_(
    ...
    map,
    'Termo Empresa',
    r.type === 'Empresa' && r.companyTermsAccepted ? 'ACEITO' : ''
  );

  put_(
    row,
    map,
    'Versão Termo Empresa',
    r.type === 'Empresa' ? r.companyTermsVersion : ''
  );
  ...
  put_(
    ...
    map,
    'Aceite Empresa',
    r.type === 'Empresa' && r.companyTermsAccepted ? now : ''
  );
  ...
  put_(
    ...
    map,
    'Assinatura HOCCO',
    r.type === 'Empresa' && r.companyTermsAccepted ? "Alex Hocc de D' Mello — Produtor & Showrunner do Reality Biográfico HOCCO" : ''
  );
  ...
  return {
    ...
    discount:
      body.discount === undefined || body.discount === ''
        ? 0
        : Math.max(0, Math.min(100, Math.round(Number(body.discount) || 0))),
    ...
    companyTermsAccepted:
      toBool_(
        body.companyTermsAccepted !== undefined
          ? body.companyTermsAccepted
          : body.companyTerms
      ),

    companyTermsVersion:
      clean_(body.companyTermsVersion || body.companyTermVersion)
...
function validateRequest_(r) {
  ...
  if (r.type === 'Empresa' && !r.companyTermsAccepted) {
    throw new Error('O Termo de Participação Empresarial precisa ser lido e aceito.');
  }

  if (r.type === 'Empresa') {
    r.companyTermsVersion = r.companyTermsVersion || '1.0-2026-09-20';
  } else {
    r.companyTermsVersion = '';
    r.companyTermsAccepted = false;
  }
  ...
  if (cfg.pricing && cfg.pricing.ranges) {
    if (!r.moment) {
      throw new Error('Selecione o momento/faixa para ' + r.modality + '.');
    }
    ...
    // Modalidades de preço fixo, como Apoiador Individual, não exigem momento/faixa.
    r.moment = '';
  }
...
function TESTE_OPERACAO_V30() {
  ...
  const pedido = registerRequest_({
    ...
    observation: 'TESTE V30 — excluir depois.', termsAccepted: true, rulesAccepted: true, companyTermsAccepted: true, companyTermsVersion: '1.0-2026-09-20'
  });
...
/*************************************************
 * SEGURANÇA WEB / ANTIABUSO
 *************************************************/

function authorizePublicRequest_(providedKey) {
  const configured = PropertiesService.getScriptProperties().getProperty('DOOX_WEB_APP_KEY');
  if (!configured) {
    throw new Error('DOOX_WEB_APP_KEY não configurada. Execute CONFIGURAR_SEGURANCA_WEB() antes de publicar o Web App.');
  }
  const a = String(configured);
  const b = String(providedKey || '');
  if (!b || a.length !== b.length || !constantTimeEqual_(a, b)) {
...
function CONFIGURAR_SEGURANCA_WEB() {
  const props = PropertiesService.getScriptProperties();
  let key = props.getProperty('DOOX_WEB_APP_KEY');
  if (!key) {
    key = Utilities.getUuid().replace(/-/g, '') + Utilities.getUuid().replace(/-/g, '');
    props.setProperty('DOOX_WEB_APP_KEY', key);
  }
  setupMVP();
  Logger.log('DOOX_WEB_APP_KEY=' + key);
  return { ok: true, message: 'Chave configurada. Copie o valor do log para a variável D O O X_WEB_APP_KEY no Vercel.', key: key };
}

function enforceRegisterRateLimit_(security) {
  const ip = String((security && security.ip) || 'sem-ip');
  const minuteKey = 'reg:m:' + Utilities.base64EncodeWebSafe(ip).substring(0, 80);
  const tenKey = 'reg:10:' + Utilities.base64EncodeWebSafe(ip).substring(0, 80);
  const minute = readCounter_(minuteKey, 60);
  const ten = readCounter_(tenKey, 600);
  if (minute > CONFIG.SECURITY.REGISTER_PER_IP_PER_MINUTE || ten > CONFIG.SECURITY.REGISTER_PER_IP_PER_10_MINUTES) {
    logSecurityEvent_(security, 'RATE LIMIT', 'ALTO', '', '', 'Excesso de solicitações de cadastro.');
    throw new Error('Muitas solicitações em pouco tempo. Aguarde alguns minutos e tente novamente.');
  }
}

function enforceTrackingRateLimit_(security, token) {
  const ip = String((security && security.ip) || 'sem-ip');
  const key = 'track:m:' + Utilities.base64EncodeWebSafe(ip).substring(0, 70);
  const count = readCounter_(key, 60);
  if (count > CONFIG.SECURITY.TRACK_PER_IP_PER_MINUTE) {
    logSecurityEvent_(security, 'RATE LIMIT ACOMPANHAMENTO', 'MÉDIO', '', '', 'Excesso de consultas de acompanhamento.');
    throw new Error('Muitas consultas em pouco tempo. Aguarde e tente novamente.');
  }
}

function readCounter_(key, ttl) {
  const cache = CacheService.getScriptCache();
  let n = Number(cache.get(key) || 0) + 1;
  cache.put(key, String(n), ttl);
  return n;
}

function isHoneypotTriggered_(raw) {
  return !!String((raw && (raw.website || raw.companyWebsite || raw.hp)) || '').trim();
}

function validateSecurityInput_(r) {
  const fields = [r.clientRequestId, r.nameOrCompany, r.name, r.company, r.whatsapp, r.email, r.profile, r.modality, r.moment, r.observation];
  fields.forEach(function(value) {
    const text = String(value || '');
    if (text.length > CONFIG.SECURITY.MAX_FIELD_CHARS) throw new Error('Um dos campos excede o limite permitido.');
    if (/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/.test(text)) throw new Error('Conteúdo inválido detectado.');
    if (/<\s*script\b|<\s*iframe\b|javascript\s*:|data\s*:\s*text\/html|onerror\s*=|onload\s*=/i.test(text)) throw new Error('Conteúdo não permitido detectado.');
  });
  if (String(r.clientRequestId || '').length > 120) throw new Error('Identificador da solicitação inválido.');
}

function logSecurityEvent_(security, event, severity, code, clientRequestId, details) {
  try {
    const ss = getSpreadsheet_();
    const sheet = getSheet_(ss, SHEETS.SEGURANCA.name);
    sheet.appendRow([new Date(), event || '', severity || 'INFO', code || '', clientRequestId || '', String((security && security.ip) || ''), String((security && security.userAgent) || '').substring(0, 500), String((security && security.action) || ''), String(details || '').substring(0, 1000)]);
  } catch (_) {}
}

function createUniqueOrderCode_(ss) {
  const sheet = getSheet_(ss, SHEETS.PEDIDOS.name);
  const map = headerMap_(sheet);
  const lastRow = sheet.getLastRow();
  const existing = new Set();
  if (lastRow >= 2 && map['Código DOOX']) {
    const vals = sheet.getRange(2, map['Código DOOX'], lastRow - 1, 1).getValues();
    vals.forEach(function(row) { const v = String(row[0] || '').trim().toUpperCase(); if (v) existing.add(v); });
  }
  for (let attempt = 0; attempt < 10; attempt++) {
    const raw = Utilities.getUuid().replace(/-/g, '').toUpperCase();
    const code = 'DX-' + raw.substring(0, 4) + '-' + raw.substring(4, 8);
    if (!existing.has(code)) return code;
  }
  throw new Error('Não foi possível gerar um Código DOOX único.');
}
...
function getPublicOrderStatus_(token, security) {

  enforceTrackingRateLimit_(security || {}, token);
  const ss = getSpreadsheet_();
  ...
  if (!pedido) {
    logSecurityEvent_(security || {}, 'TOKEN INVÁLIDO', 'MÉDIO', '', '', 'Tentativa de acompanhamento com token inválido.');
    throw new Error('Pedido não encontrado ou token inválido.');
  }

  logSecurityEvent_(security || {}, 'ACOMPANHAMENTO CONSULTADO', 'INFO', pedido.code, '', 'Consulta autorizada pelo token.');
